...
curl --location --request POST "https://dev.api.leavesmc.xor7.cn/v2/commit/build" --header "Content-Type: application/json" --header "Authentication: Bearer $API_V2_TOKEN" --data-raw "{\"project_id\":\"$project_id\",\"version\":\"$mcversion\",\"channel\":\"$channel\",\"changes\":\"$changes\",\"jar_name\":\"$jar_name\",\"sha256\":\"$jar_sha256\",\"tag\":\"$tag\"}"
